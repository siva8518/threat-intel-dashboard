...
import { fetchDetectionBacklog, setDetectionBacklogStatus, clearDetectionBacklogStatus, draftDetectionBacklogArtifacts } from "@/api/dashboardApi";
...
export function useDetectionBacklog() {
  ...
  // Drafting all 12 artifacts in one completion is a several-second-to-a-
  // minute Groq round trip triggered one item at a time from the table --
  // draftingId (not just isPending) lets the UI show a per-row "Drafting…"
  // state instead of disabling every row's button at once while any one
  // draft is in flight.
  const draftArtifactsMutation = useMutation({
    mutationFn: draftDetectionBacklogArtifacts,
    onSuccess: () => queryClient.invalidateQueries({ queryKey: queryKeys.detectionBacklog }),
  });
  ...
  return {
    ...
    draftArtifacts: draftArtifactsMutation.mutateAsync,
    draftingId: draftArtifactsMutation.isPending ? draftArtifactsMutation.variables : null,
  };
}
