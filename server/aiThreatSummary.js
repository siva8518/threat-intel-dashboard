...
const SYSTEM_PROMPT =
  ...
  '}\n' +
  '"operationalRecommendations": array of role-specific action items across exactly these 6 teams -- "Threat Intelligence", "Threat Hunting", "Detection Engineering", "SOC Operations", "Vulnerability Management", "Incident Response" -- each shaped {"team": string (copied exactly from that list), "priority": "Critical"|"High"|"Medium"|"Low", "recommendation": string (MUST begin with an action verb -- Track, Hunt, Validate, Correlate, Update, Review, Create, Monitor, Search, Block, Escalate, Isolate, Contain, Audit, Investigate, Scan, Deploy, etc. -- specific and operational, e.g. "Hunt for internet-facing instances of the affected service exposed before the patch date" not "monitor for suspicious activity"), "rationale": string (exactly one sentence explaining why THIS threat specifically supports this action)}. For the "Threat Intelligence" team specifically, prefer concrete collection/correlation actions over restating the trend narrative already covered in operationalActions.threatIntelTakeaway below -- e.g. "Track exploitation/PoC reports for this CVE across additional vendor advisories and OSINT sources", "Correlate the extracted IOCs against existing threat intel platform/watchlist entries for reuse across other investigations", "Cross-reference this vulnerability/technique against known threat actor TTPs for early-warning correlation", "Assess whether this activity overlaps with an already-tracked campaign or actor", "Share confirmed IOCs with industry ISAC/peer-sharing channels" -- concrete enough that a CTI analyst could act on it today, not a one-line summary of what the article already said. This is a distinct, flat, prioritized CHECKLIST view across all six teams, not a restatement of operationalActions above -- distill and prioritize that guidance into short, scannable action items rather than repeating its sentences verbatim. Generate a recommendation for a team ONLY if the article\'s actual reported threat activity genuinely supports it -- never generic advice like "monitor for suspicious activity" or "apply patches." Do not invent a plausible-sounding action a team could theoretically take on any threat; ground every recommendation in what this specific article describes. If a team genuinely has no article-supported action, include exactly ONE entry for that team: {"team": that team, "priority": "Low", "recommendation": "No additional actions identified", "rationale": a one-sentence reason why (e.g. "Article contains no indicators or activity actionable by this team")}. A team may have multiple entries (up to 4) if the article genuinely supports several distinct actions for it; most single-CVE-advisory articles will not.\n' +
...
function safePlatformRecommendations(v) {
  ...
  return {
    ...
    userRecommendations: safeArray(v.userRecommendations),
  };
}
...
export function extractIocs(text, attackTechniques) {
