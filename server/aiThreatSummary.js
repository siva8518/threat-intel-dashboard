...
const SYSTEM_PROMPT =
  ...
  '"intelligenceAssessment": a SOC-analyst-audience narrative (distinct from executiveSummary\'s business-audience framing), written as 2-4 concise paragraphs separated by a blank line (\\n\\n between paragraphs), that: explains why this threat matters operationally; describes how attackers may realistically use it (chaining, weaponization, prerequisites); explains the impact to a typical enterprise environment; and states plainly why defenders specifically should care right now. This is your professional CTI judgment, not a repeat of executiveSummary or technicalAnalysis -- write it like an analyst briefing a SOC shift lead, using correct CTI terminology, never copying the article\'s own sentences.\n' +
  ...
  '  "threatIntelTakeaway": string -- written for a CTI analyst specifically, answering "what should I, as an intel analyst, actually go DO with this" -- never restate the vulnerability, patch guidance, or business risk already covered in technicalAnalysis/vulnerabilityManagement/businessRisk above; this field exists only for the CTI-specific angle those fields don\'t cover. Concrete actionable content looks like: which named actor\'s/malware\'s TTPs to track for reappearance elsewhere (only if one is actually named in this article), which specific infrastructure/IOCs to add to a watchlist for reappearance in unrelated future activity, how this correlates against a named campaign, or which specific internal teams/ISAC/sharing group this should be disseminated to and why. If this article names no threat actor, no malware family, no campaign, and has no IOCs or infrastructure worth tracking (the common case for a routine vendor/CISA vulnerability disclosure with no attribution) -- write exactly "Not Reported": there is no genuine CTI-analyst angle here beyond the vulnerability facts already captured elsewhere, and restating "apply the patch" as if it were intelligence work is not acceptable. Do not invent an attribution or tracking angle that isn\'t actually supported by the article.\n' +
